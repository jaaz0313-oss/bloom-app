...
type ClienteDescargarProyeccionExcelButtonProps = {
  bodaId: string;
  inline?: boolean;
  onError?: (message: string | null) => void;
};

export function ClienteDescargarProyeccionExcelButton({
  bodaId,
  inline = false,
  onError,
  ...
  async function handleDownload() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(payload?.error ?? t.downloadProjectionError);
      }

      const blob = await response.blob();
      const disposition = response.headers.get("Content-Disposition");
      const filenameMatch = disposition?.match(/filename="([^"]+)"/);
      const filename = filenameMatch?.[1] ?? "Proyeccion.xlsx";
