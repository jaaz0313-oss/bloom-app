...
import { getComprobantesPagoFolderUrl } from "@/lib/google-drive";
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  try {
    const folderUrl = await getComprobantesPagoFolderUrl(id);
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "No se pudo obtener la carpeta de comprobantes.",
      },
      { status: 500 },
    );
  }
