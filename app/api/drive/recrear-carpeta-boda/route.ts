...
import { recreateDriveFolderForBoda } from "@/lib/google-drive";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const folder = await recreateDriveFolderForBoda(
      bodaId,
    ...
    if (!folder.folder_url) {
      return NextResponse.json(
        { error: "La carpeta se recreó pero Google no devolvió un enlace." },
        { status: 500 },
      );
    }
    ...
    await registrarAccion({
      ...
      accion: AUDITORIA_ACCIONES.DRIVE_CARPETA_RECREADA,
      entidad: "boda",
      entidadId: bodaId,
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "No se pudo recrear la carpeta en Drive.",
      },
      { status: 500 },
    );
  }
